...
class reaction_services:
    ...
    @classmethod
    def send_comment_reaction(cls, comment_id, user_id,emoji_native):
        ...
        try:
            # 投稿されたリアクションがテーブルにあるか確認
            is_already = Reactions.is_already_reaction_for_comment(conn, comment_id, user_id)
            should_insert = True    

            if is_already: #すでに押した(リアクション済み)の場合
                    Reactions.delete_comment_reaction(conn, comment_id, user_id) # 既にあるものをもう一度押した→取り消しの処理

                    if is_already['emoji_type'] == emoji_native: # 過去に押したものと同じなら
                        should_insert = False # 登録はしない
            if should_insert:
                    Reactions.insert_comment_reaction(conn, comment_id, user_id, emoji_native) # 新しくリアクションを登録する

            conn.commit() # 全部うまくいったらコミット
            
            # 現在の最新のリアクション情報を再取得してフロントに返す
            reactions = []
            current_reactions_info = Reactions.get_comment_reactions_by_id(comment_id)
